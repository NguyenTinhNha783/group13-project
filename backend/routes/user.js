...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        const resetUrl = `http://localhost:3001/users/reset-password/${resetToken}`;
...
router.post('/upload-avatar', authMiddleware, parser.single('avatar'), async (req, res) => {
    try {
        ...
        user.avatar = req.file.path; // ✅ Lưu link Cloudinary đầy đủ
        await user.save();

        console.log('Avatar mới:', user.avatar);

        // Trả về thông tin user mới nhất cho frontend
        res.json({
            ...
            user: {
                ...
                role: user.role,
                avatar: user.avatar,
                user, // ✅ đảm bảo frontend có avatar khi reload
            },
        });
    } catch (err) {
